...
We will use pip (\url{https://pypi.org/project/pip/} for long-term release support. Distribution via pip is the most widely used, and easy for users, approach to distributing Python modules; such projects as TensorFlow, scipy, and pandas all use pip as their primary distribution method.

To promote long-term sustainability, continuous integration tests will be developed for all aspects of the project, using Travis CI (\url{https://travis-ci.org/}) and pytest (\url{https://docs.pytest.org/en/latest/}). In addition, under co-PI Groce's guidance, we will use automated testing with TSTL (\url{https://github.com/agroce/tstl}) and Hypothesis (\url{https://github.com/HypothesisWorks/hypothesis}), to ensure our code works correctly even given unexpected inputs.  CI will also perform static analysis, using pyflakes (\url{http://www.blog.pythonlibrary.org/2012/06/13/pyflakes-the-passive-checker-of-python-programs/}) for lightweight bug detection and PyLint (\url{https://www.pylint.org/}) and pycodestyle (\url{https://github.com/PyCQA/pycodestyle}) for more rigorous fault detection and enforcement of coding standards to ensure long-term maintainability.
We will summarize these processes, and provide reference to existing documentation, so that future developers can adhere to the coding and design standards we develop.

All changes to the core source code will be made through a standardized, best-practice approach to open source software: first an issue will be added to the GitHub issue tracker for the project, then a Pull Request in a new branch will be used to develop the fix or new feature.  No changes will be merged into the main branch of the software without at least one code review by a senior member of the project, and without passing all tests.  Code review will also enforce the rule that new features must add corresponding new tests.  The GitHub source version will only release stable versions to pip, with various additional manual test and review steps to ensure these versions are ready for release.  This process is one that co-PI Groce has used and orchestrated as lead developer in the past, for multiple complex open source projects, including a testing framework, DeepState (\url{https://github.com/trailofbits/deepstate}), in collaboration with Trail of Bits, where more than ten developers contribute to a popular tool (more than 400 GitHub stars) with code in multiple languages (Python, C, and C++).
We will create a GitHub Wiki to document our code, in addition to thorough documentation within the Python source code. The documentation will provide information on the overall software architecture, key design principles, and examples of how different capabilities of the software can be used.  The wiki will also demonstrate how developers can define new hemodynamic measures and extend our software in other ways.

We will also develop benchmarks to validate our software. Since our software  primarily deals with data defined on surface meshes, we will design validation benchmarks based on spheres and cylinders where analytical differential geometry calculus could be performed. Namely, analytically defined surface vector fields on a sphere and cylinder will be defined based on polar coordinates. The surface vector fields will be defined such that they vary in space and time. We will also create discrete surface meshes on the same sphere and cylinder, and include the same surface vector fields as discrete data. Several measures (such as gradients, near-wall transport, etc) will be computed and compared to the analytical results. 
...
Software,''  (9/2012-9/2015). {\bf Intellectual Merit:} The results of CCF-1217824 included a
number of advances to practical automated generation of software tests, resulting in a Best Paper Award in 2014 at the IEEE International Conference on Software Testing, ICST~\cite{icst2014}. {\bf Broader Impact:} CCF-1217824 resulted in numerous publications at top venues in software testing and analysis~\cite{Onward14,PLDI13,issta14,icst2014,helphelp,DirectedSwarm,stvrcausereduce,tstlsttt,ISSTA15,ASEAdeq,NFM15}, these results were key in discovering important bugs in major open source projects.
Tools and data sets from CCF-1217824 are available via GitHub, and used the same basic approach to software quality and maintenance, using CI, PRs, and code review, as suggested in this proposal (e.g. \url{https://github.com/agroce/tstl}).
